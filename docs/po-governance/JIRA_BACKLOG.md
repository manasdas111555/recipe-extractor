...
| **UPA-1239** | Dedicated OCI Staging Backend Infrastructure & Environment Isolation | DevOps / Infrastructure / Security | P0 | Governs dedicated OCI Staging backend infrastructure setup and environment isolation: (1) OCI second-instance feasibility/capacity verification (VM.Standard.E2.1.Micro, AMD, 1 GB RAM, Always Free eligible; CPU allocation/capacity to be verified in OCI Console as external verification); (2) Canonical Ubuntu 24.04 LTS OS image; (3) Memory configuration (1 GB physical RAM with proposed 2 GB `/swapfile` as a memory-pressure/OOM mitigation measure only, without equating swap to physical RAM); (4) Staging network topology, VCN subnet, Security List / firewall design (Public inbound 80/443; restricted SSH port 22; no public FastAPI port 8000; no public Redis port 6379); (5) Initial runtime stack limited to Caddy reverse proxy (`caddy:2-alpine`) and FastAPI gateway (`api`) with `ALLOW_DB_WRITES=false` initially; (6) Redis/Celery background processing explicitly DEFERRED for initial Staging backend infrastructure deployment and preserved for a later governed Gate 8 (initial VM deployment limited to API gateway and Caddy unless later governed gate authorizes workers/Redis); (7) Staging-only credentials (`ENVIRONMENT=staging`, `SECRET_KEY`, `SUPABASE_URL=https://mzpkdmaxsuhwezsooidu.supabase.co`); (8) Supabase Staging target (`mzpkdmaxsuhwezsooidu.supabase.co`) with zero Production DB (`scrqvbgjybnrvcpxbygf`) contact; (9) Automatic TLS certificate termination via Caddy; (10) Container image rollback strategy with zero automated DB resets; (11) Recorded verification evidence (commit hash, secret presence audit, Caddy HTTPS health check, PostgREST 404 lookup, Production isolation); (12) Rule 21 documentation synchronization across `SYSTEM_ARCHITECTURE.md`, `ENVIRONMENTS.md`, `ORACLE_CLOUD_DEPLOYMENT.md`, `DISASTER_RECOVERY.md`, `JIRA_BACKLOG.md`. Explicitly excludes Production changes, Vercel changes, Supabase changes, Redis/Celery activation, E2E execution, UPA-1226 remediation, commit rewriting, and recovery branch deletion.<br>**Affected files**: `docs/po-governance/JIRA_BACKLOG.md`, `docs/architecture/SYSTEM_ARCHITECTURE.md`, `docs/architecture/ENVIRONMENTS.md`, `docs/architecture/ORACLE_CLOUD_DEPLOYMENT.md`, `docs/architecture/DISASTER_RECOVERY.md`.<br>**Applicable AGENTS.md rules**: Rules 2, 6, 9, 14, 16, 17, 21, Sections 18, 19, 20.<br>**Risks & Rollback**: OCI capacity limitations in region `ap-hyderabad-1` or Staging environment misconfiguration. Rollback: Container image rollback on Staging VM; zero automated DB resets.<br>**Test Plan**: 1. Verify OCI capacity & VM creation. 2. Verify OS swap & Docker/Compose setup for Caddy + API. 3. Audit environment variables (`ALLOW_DB_WRITES=false`). 4. Verify Caddy HTTPS health & public port isolation. 5. Perform PostgREST 404 lookup against Staging Supabase. 6. Confirm zero Production DB contact. | 🟡 In Progress | Uncommitted | Read-only OCI discovery active | Pending PO |
...
| **UPA-1239** | Dedicated OCI Staging Backend Infrastructure & Environment Isolation | DevOps / Infrastructure / Security | P0 | Governs dedicated OCI Staging backend VM setup (VM.Standard.E2.1.Micro, AMD, 1 GB RAM, Always Free eligible; CPU allocation/capacity to be verified in OCI Console), Ubuntu 24.04 LTS OS, 1 GB RAM with proposed 2 GB `/swapfile` for memory-pressure/OOM mitigation only, Security List/firewall rules (Public inbound 80/443; restricted SSH port 22; no public 8000; no public 6379), Docker Engine/Compose initial runtime limited to Caddy reverse proxy (`caddy:2-alpine`) and FastAPI gateway (`api`) with `ALLOW_DB_WRITES=false` initially, Redis/Celery background workers explicitly DEFERRED for initial Staging deployment and preserved for a later governed Gate 8, Staging credentials (`ENVIRONMENT=staging`), Supabase Staging target (`mzpkdmaxsuhwezsooidu.supabase.co`), zero Production DB contact, E2E execution deferred, Caddy TLS termination, container rollback strategy, PostgREST 404 verification evidence, and Rule 21 documentation synchronization across `SYSTEM_ARCHITECTURE.md`, `ENVIRONMENTS.md`, `ORACLE_CLOUD_DEPLOYMENT.md`, `DISASTER_RECOVERY.md`, `JIRA_BACKLOG.md`. | 🟡 In Progress | Uncommitted | Read-only OCI discovery active | Pending PO |
